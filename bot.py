        ...
        for msg in Message.select().where(Message.date_send.is_null()).order_by(Message.id):
            try:
                if msg.file_type == 'document':
                    ...
                    try:
                        try:
                            context.bot.send_message(msg.to_user, msg.text[0:4000].strip(), parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True)
                        except Exception as e:
...
def matches(update, context):
    ...
    middle = []
    try:
        resp_t = requests.get('http://' + bot_prop.IP_SERVER + '/get_middle', timeout=500)
